...
class AndNot_network:
    def __init__(self):
        ...
        self.toposorted = (
            []
        )  # list sized N+1; toposorted[i] = node id to compute at step i
    ...
    def getvalue(self, id_):
        """Return boolean value for a (possibly negated) id."""
        c = False
        if id_ < 0:
            # negated reference
            assert -id_ <= self.N
    ...
    def calculatenetwork(self):
        ...
        for i in range(INPUT_NODES + 1, self.N + 1):
            ...
            self.value[target_id] = self.getvalue(self.C_1[i]) & self.getvalue(
                self.C_2[i]
            )
    ...
    def guess(self, correct):
        ...
        # sort descending by (on_count, category_index)
