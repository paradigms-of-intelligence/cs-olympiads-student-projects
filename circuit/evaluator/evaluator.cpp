...
struct AndNot_network {
    ...
    void init() {
        ...
        N = __read_int32_t(graphinput)+1;
        value.resize(N); C_1.resize(N); C_2.resize(N);

        for (int i = 0; i < N; i++) {
            int id = __read_int32_t(graphinput);
            C_1[id] = __read_int32_t(graphinput);
            C_2[id] = __read_int32_t(graphinput);
        }

        result_nodes.resize(10);
        for (int i = 0; i < 10; i++) result_nodes[i] = __read_int32_t(graphinput);
        graphinput.close();
    }
